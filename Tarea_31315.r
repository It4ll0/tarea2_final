...
separate_eight_day_composite = function(x, fechas){
  ...
  # no considerar la ultima imagen del aÃ±o
  ...
  # guardar las imagenes que abarcan solo 1 mes dentro de la composicion de 8 dÃ­as
  r2 = x[[-index]]
  ...
  # ciclo de 1 a n
  for (i in 1:n) {
    # obtener la fecha del Ãºltimo dia que abarca la serie de 8 dÃ­as
    fecha2 = fechas_img[i] %m+% days(7)
    
    # Multiplicamos la imagen por la proporcion de dÃ­as que abarca de cada mes
    # mes actual
...
ggplot(data.year)+
  ...
  labs(x = "tiempo", y = "(mm)", title = "Serie de tiempo mensual de componentes del BH",
       subtitle = "Los años sin medicion de caudal, faltan datos en algunos meses",
       color = "")
